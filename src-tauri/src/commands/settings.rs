...
pub fn get_library_root(state: State<'_, AppState>) -> Result<String, String> {
    Ok(library_root_for_app_state(&state)
        .to_string_lossy()
        .to_string())
}

#[tauri::command]
pub fn set_library_root(path: String, state: State<'_, AppState>) -> Result<String, String> {
    let trimmed = path.trim();
    if trimmed.is_empty() {
        return Err("Library folder cannot be empty.".to_string());
    }
    let path = PathBuf::from(trimmed);
    if !path.is_absolute() {
        return Err("Library folder must be an absolute path.".to_string());
    }
    fs::create_dir_all(&path)
        .map_err(|error| format!("Failed to create library folder: {error}"))?;
    let value = path.to_string_lossy().to_string();
    lock_store(&state)?.set_config("library_root", &value)?;
    logging::log_event(
        &state.data_dir,
